...
from pyspark.sql.functions import lower, concat, col, regexp_replace
...
df = (
    ...
    .csv(source_file)
    .withColumn(
        "fullName", lower(concat(col("firstName"), col("middleName"), col("lastName")))
    ).withColumn(
        "ssn", regexp_replace(col("ssn"),"-","")
    )
).dropDuplicates(["fullName"]).drop("fullName")
df.rdd.getNumPartitions()
df_write = df.coalesce(1)
df_write.rdd.getNumPartitions()
df_write.write.save(delta_dest_dir)
